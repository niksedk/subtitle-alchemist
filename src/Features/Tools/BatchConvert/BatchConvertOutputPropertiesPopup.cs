...
public sealed class BatchConvertOutputPropertiesPopup : Popup
{
    public BatchConvertOutputPropertiesPopup(BatchConvertOutputPropertiesPopupModel vm)
    {
        BindingContext = vm;
        ...
        grid.Add(labelOverwrite, 0, 3);

        var switchOverwrite = new Switch
        {
            HorizontalOptions = LayoutOptions.Start,
            VerticalOptions = LayoutOptions.Center,
            Margin = new Thickness(0, 25, 10, 0),
        }.BindToggledProperty(nameof(vm.Overwrite));
        grid.Add(switchOverwrite, 1, 3);
        grid.SetColumnSpan(switchOverwrite, 2);
